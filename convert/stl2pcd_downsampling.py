...
def main(origin_path, stl_path, output_path, N, spacing_type, spacing=[0.5, 0.5, 0.5]):
    filled_pcd_path = os.path.join(output_path + f"\\pcd(filled-1-{N})")
    surface_pcd_path = os.path.join(output_path + f"\\pcd(surface-1-{N})")
    ...
    for stl_root in rootlist:
        ...
        filled_pcd_root = os.path.join(filled_pcd_path + "\\" + filename + f"(filled-1-{N}).pcd")
        surface_pcd_root = os.path.join(surface_pcd_path + "\\" + filename + f"(surface-1-{N}).pcd")
        print(filename)
        ...
        pre_spacing = copy.deepcopy(spacing)
        post_spacing = [pre_spacing[0]*N**(1/3), pre_spacing[1]*N**(1/3), pre_spacing[2]*N**(1/3)]
        print(f"{pre_spacing} -> {post_spacing}")
        ...
        "stl -> surface pcd"
        surface_pcd_data = stl_to_surface_pcd(stl_root, post_spacing)
        save_pcd(surface_pcd_data, surface_pcd_root)

        "stl->vti（中空が埋まっている）"
        vti_data = stl_to_vti(stl_root, post_spacing, origin)
        "vti->numpy"
        vti_array = vtk_to_numpy(vti_data)
        "closing処理（アノテーションデータは非推奨モデルの場合は推奨）"
        vti_array = closing(vti_array, cube(100))
        "numpy->filled pcd"
        filled_pcd_data = vti_to_pcd(vti_array, post_spacing, origin)
        save_pcd(filled_pcd_data, filled_pcd_root)

if __name__ == '__main__':
    """spacing
    VTI:VTIから取得. Originにvtiのpathを渡してください
    MHD:MHDから取得. Originにmhdのpathを渡してください
    manual:自分で設定、spacingに渡すこと,もし渡さない場合は[0.5, 0.5, 0.5]になります"""
    origin_path = r"D:\takahashi_k\registration(model)\original\vti"
    stl_path = r'D:\takahashi_k\registration(model)\original\stl'             
    output_path = r"D:\takahashi_k\registration(model)\original"
    #dicom_path = r"D:\takahashi_k\database\us\20240502_YAMAGUCHI\DICOM"
    spacing_type = "manual"
    N = 2                                           #密度は1/Nになります
    
    main(origin_path, stl_path, output_path, N, spacing_type, spacing=[0.5, 0.5, 0.5])
