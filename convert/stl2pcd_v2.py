...
def stl_to_surface_pcd(stl_root, spacing, origin):
    print(f"loading...\n{stl_root}")
    ...
    surface_points = surface_points + np.asarray(origin)
...
def main(stl_path, Out_path, spacing, origin):
    filled_pcd_path = os.path.join(Out_path + "\\PCD(filled)")
    surface_pcd_path = os.path.join(Out_path + "\\PCD(surface)")
    ...
    for stl_root in rootlist:
        ...
        "stl -> surface pcd"
        surface_pcd_data = stl_to_surface_pcd(stl_root, spacing, origin)
...
if __name__ == '__main__':
    """[0,0,0]のoriginで作成したstlをvtiに沿添えたい場合"""
    ...
    Out_path = r'D:\takahashi_k\database\us\20240502_YAMAGUCHI\Annotation'  
    ...
    for stl_root in rootlist:
        ...
        main(stl_path, Out_path, spacing, origin)
