...
def task(x,y,z, i,j,k, density_array, origin_kernel_3d, shared_mem_name, dnsFiled_array_shape):
    dnsFiled_array_shm = shared_memory.SharedMemory(name=shared_mem_name)
    dnsFiled_array = np.ndarray(dnsFiled_array_shape, dtype=np.uint8, buffer=dnsFiled_array_shm.buf)
...
def calculate_densityFiled(density_array):
    ...
    #共有メモリ作成
    # density_array_shm = shared_memory.SharedMemory(create=True, size=np.prod(density_array.shape) * np.dtype(np.float64).itemsize)
    # density_array_sh = np.ndarray(density_array.shape, dtype=np.float64, buffer=density_array_shm.buf)
    # origin_kernel_3d_shm = shared_memory.SharedMemory(create=True, size=np.prod(origin_kernel_3d.shape) * np.dtype(np.float64).itemsize)
    # origin_kernel_3d_sh = np.ndarray(origin_kernel_3d.shape, dtype=np.float64, buffer=origin_kernel_3d_shm.buf)
    #print(np.prod(dnsFiled_array.shape) * np.dtype(np.float64).itemsize)
    sz = dnsFiled_array.shape[0] * dnsFiled_array.shape[1] * dnsFiled_array.shape[2] * np.dtype(np.float64).itemsize
    dnsFiled_array_shm = shared_memory.SharedMemory(create=True, size=sz)
    #dnsFiled_array_sh = np.ndarray(dnsFiled_array.shape, dtype=np.float64, buffer=dnsFiled_array_shm.buf)
    dnsFiled_array = np.ndarray(dnsFiled_array.shape, dtype=np.float64, buffer=dnsFiled_array_shm.buf)
    # i_mp = mp.Value("i", i)
    # j_mp = mp.Value("i", j)
    # k_mp = mp.Value("i", k)
    processes = [mp.Process(target=task, args=(x,y,z, i,j,k, density_array, origin_kernel_3d, dnsFiled_array_shm.name, dnsFiled_array.shape)) for x in range(i) for y in range(j) for z in range(k)]
    ...
    dnsFiled_array_shm.close()
    dnsFiled_array_shm.unlink()
